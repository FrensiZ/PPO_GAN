...
def pretrain_generator(generator, optimizer, pre_epoch_num, batch_size, 
    ...
    with th.no_grad():
        generated_sequences = generator.generate(len(test_data))
...
def generate_ppo_samples(model, start_token_distribution, sequence_length, num_samples, device):
...
def evaluate_best_model(model, output_path, test_data, vocab_size, sequence_length, start_token_distribution, device):
    ...
    test_samples = generate_ppo_samples(model, start_token_distribution, sequence_length, len(test_data), device)
